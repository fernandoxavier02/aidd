...
### Provider support matrix (guard × provider × moment)

The seven fiscal guards share a provider-neutral core (`.claude/hooks/lib/guards/` — one rule module per guard, contract `event → verdict {allow|warn|deny}`); each provider gets a thin adapter that only translates protocol. Enforcement per layer:

| Guard | Claude Code (edit-time) | Codex (edit-time) | git pre-commit (commit-time) |
|---|---|---|---|
| contract | deny | best-effort (warn-tier) | deny |
| phase | warn/deny (config) | — | — |
| tdd | deny (RED-gate, opt-in) | — | warn (evidence policy, opt-in) |
| secrets | deny | best-effort (warn-tier) | deny |
| domain | deny (opt-in map) | best-effort (warn-tier, opt-in map) | deny (opt-in map) |
| rls | deny (opt-in) | best-effort (warn-tier, opt-in) | deny (opt-in) |
| frontend-business | warn | best-effort (warn-tier, opt-in map) | warn |

- **Claude Code** — hooks wired via `.claude/settings.json` (PreToolUse). Full edit-time enforcement, including session env overrides for supervised sessions.
- **Codex** — adapter (`.claude/hooks/lib/adapters/codex.cjs`) registered in the consumer's `.codex/hooks.json`, generated by `aidd init` (never shipped in the tarball). ALL Codex edit-time enforcement is **best-effort**: the 2026 Codex hook-trust model means a registered hook only runs after the user explicitly trusts it. Never rely on this layer for guarantees — the git net is the backstop.
- **git pre-commit** — the universal net (`.claude/hooks/lib/git-net.cjs`), installed by `aidd init` into the **effective** hooks dir (`git rev-parse --git-path hooks`, honoring `core.hooksPath`), scanning **staged blobs** only (`git show :0:`). Per-clone: each fresh clone needs `aidd init`/`aidd update` to re-arm the net. Verdict mapping: deny → exit 1 (commit blocked); warn → stderr, commit proceeds. Session env bypasses (`AIDD_V2_INSTALL`, `AIDD_TDD_OVERRIDE`, `AIDD_OVERRIDE_*`) are **not** honored at commit time; content byte-identical to what `aidd init/update` itself installed (manifest hash match) is tool-managed and skipped.
- **Session machinery is Claude Code-only** and outside this matrix: `aidd-session-bootstrap.cjs` (SessionStart), `aidd-sensor.cjs` (PostToolUse), `aidd-stop-rules-preserver.cjs` (PreCompact), `aidd-adversarial-read-guard.cjs` (subagent read isolation) — these moments have no portable equivalent.

This matrix is mechanized: `tests/hooks/support-matrix-integrity.test.cjs` fails whenever a cell above promises enforcement that the delivered adapters do not actually register.
