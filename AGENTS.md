...
- **Hooks (partial parity with Claude Code):** Claude Code has `.claude/hooks/` fully active (context injection, error logging, and the 7 fiscal guards at edit-time). Codex gets a **warn-tier PreToolUse adapter** registered in `.codex/hooks.json` by `aidd init`, covering the portable guards: **contract, secrets, domain, rls, frontend-business** (tdd/phase and the session machinery remain Claude-only). This layer is **best-effort** — Codex's hook-trust model means it only runs after you trust the hook — so the Codex agent must STILL follow the rules manually (read MUST READ at session start, update `VERIFICATION_REPORT.md` on errors, respect core immutability), and the **git pre-commit net** is the guaranteed enforcement backstop. Full matrix: `AIDD.md` → "Provider support matrix".
