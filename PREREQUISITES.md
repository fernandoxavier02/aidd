...
| **Claude Code** (for full hook automation) | The `.claude/hooks/` automation is wired through `.claude/settings.json`, which Claude Code reads. | Codex gets a **warn-tier** PreToolUse adapter (`.codex/hooks.json`, generated by `aidd init`; best-effort — trust-gated), and any git repo gets the **pre-commit net** (guaranteed commit-time layer). Full guard × provider × moment matrix: `AIDD.md`. |
