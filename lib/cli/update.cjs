...
const { updateGitHooksLayer } = require("./git-hooks.cjs");
...
function update(opts = {}) {
  ...
  // Camadas registradas no manifesto (legado sem `layers` = só claude).
  ...
  // Bootloader blocks — CLAUDE.md (claude); AGENTS.md (claude ou codex).
  for (const name of fm.BOOTLOADERS) {
    const wanted = name === "CLAUDE.md" ? layers.claude !== false : (layers.claude !== false || layers.codex);
    if (!wanted) continue;
  ...
  // Camada codex — merge idempotente do hooks.json.
  if (layers.codex) {
    installCodexLayer(targetRoot, manifest, results, { mode, dryRun });
  }

  // Camada git — regenerar com marcador intacto; recusar mão-editado (R1 f14).
  if (layers.git) {
    updateGitHooksLayer(targetRoot, manifest, results, { mode, dryRun });
  }
